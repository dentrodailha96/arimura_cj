<?php
    ...
    if (!empty($_REQUEST['p_price_product'])) {
        $price = $_REQUEST['p_price_product'];
        $updates[] = "price_product = '$price'";
    }
            
    if (!empty($_REQUEST['p_sales_unit'])) {
        $sales_unit = $_REQUEST['p_sales_unit'];
        $updates[] = "sales_unit = '$sales_unit'";
    }
    ...
    // Join all updates with commas
        ...
        $textsql = "UPDATE arimura_cj.products 
                   SET $update_string 
                   WHERE id_product = '$id'";
